...
float target = 460, ang_control = 0, control = 0, v1 = 0, v2 = 0, v3 = 0;
...
float Kp1 = -0.15, Kd1 = -0.1;
float Kp2 = 0.2, Kd2 = 0.2;
...
void loop()
{
  ...
  {
    Kp1 = -0.22, Kd1 = -0.49;      //float Kp1=-0.225,Kd1=-0.65;
    Kp2 = 0.085, Kd2 = 0.29;
  }
  ...
  if (junc == false)
  {
    if ((state == 0) || (state == 2) || (state == 5) || (state == 9))
    {
      ...
      if (state == 2)
      {
        if (abs(xEnc.read() - junc_pos) > 4200)
          acceleration = -42;
      ...
      if (state == 5)
      {
        if (abs(xEnc.read() - junc_pos) < 3000)
          acceleration = 21;
      }
    }

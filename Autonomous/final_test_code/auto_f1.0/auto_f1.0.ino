...
float Kp1_y_rev = 0.60, Kd1_y_rev = 0.2366;
float Kp2_y_rev = 0.35, Kd2_y_rev = 0.3;
...
long min_base_val[40] =           {   0,     0,      0,      0,    0,    0,  -1450,   -840,     0,    0,  -500,    0,  -700,     0,     0, -900,     0,    0,    400};
long max_base_val[40] =           { 400,  2000,   1000,      0,  500,    0,      0,   1000,  1450,    0,     0,    0,     0,     0,     0,  -400,   300,    0,   1800};
long acceleration_val[40] =       {   4,     4,     10,      0,   10,    0,    -11,     10,    11,    0,   -10,    0,    -6,     0,     0,    -6,    15,    0,      8};
long deacceleration_val[40] =     {   0,   -14,    -10,      0,   -2,    0,     10,    -10,   -10,    0,     2,    0,     0,     0,     0,     6,     0,    0,     -9};
int dis_2_slow[40] =              {   0, 15000,   6000,      0,    0,    0,   4000,   6000,  6000,    0,     0,    0,  3000,     0,     0, 16000,     0,    0,  16000};
int time_2_slow[40] =             {   0,  2900,   1300,      0,  500,    0,   1000,   1500,  1000,    0,   500,    0,  1000,     0,     0,  1800,     0,    0,   2300};
...
int manual_dis_min = 5, manual_dis_max = 100, manual_reverse_dis_min_1 = 250, manual_reverse_dis_max_1 = 1023, ball_dis_min = 5, ball_dis_max = 28;
...
int lsa_margin = 80, lsa_control = 0;
...
int TZ1_dis_2_move = 1400, TZ2_dis_2_move = 800, dis_margin =300, encoder_control = 0;
...
//      write_vel();
  ...
  else if (state == 16)
  {
    ...
    else if (absolute(analogRead(LSA_0) - target) > lsa_margin)
    {
      while(absolute(analogRead(LSA_0) - target) > lsa_margin){
        delay(1);
        int aa = analogRead(LSA_0);
        lsa_control = (target - aa)/4;
        delay(1);
  ...
  else if (state == 32)         // retry
  {
    ...
    else if (millis() - state_time_cnt > time_2_slow[state]+500)
    {
        state_change();
    }
    update_vel();

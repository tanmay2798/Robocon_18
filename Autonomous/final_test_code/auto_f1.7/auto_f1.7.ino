...
long min_base_val[40] =           {   0,     0,      0,      0,    0, -850,  -1450,   -840,     0,    0,  -500,    0,  -700,     0,     0, -1400,     0,    0,    400,   -200};
...
/////////////////////////// Throw //////////////////////
int golden_rack_w = 90, adjust_rack_w = 40;
...
int time_cnt = 0;
///////////////////////////// Distances /////////////////////////////////////
int TZ1_dis_2_move = 2100, TZ2_dis_2_move = 1400, dis_margin =300; 
  ...
  else if (state == 16)
  {
    ...
    else if (absolute(analogRead(LSA_0) - tz3_align_val) > lsa_margin)
    {
      while(absolute(analogRead(LSA_0) - tz3_align_val) > lsa_margin){
        ...
        pLSA_0_val = LSA_0_val;
        lsa_control = (tz3_align_val - LSA_0_val);
        delay(1);
...
//        Serial.print("Speed up");
  ...
  else if (state == 32)         // retry
  {
    ...
    if (absolute(xEnc.read() - junc_pos_x) > 15000)
    {
      stop_bot();
      state_change();
    }
  ...
  else if (state == 93)
  {
    ...
    //////// LS Mech 1 //////////////
    if (digitalRead(ls_sol) == HIGH || digitalRead(ls_mag) == HIGH){
    ...
    //////// LS Mech 2 //////////////
    if (digitalRead(ls_sol_r) == HIGH || digitalRead(ls_mag_r) == HIGH){

...
int LS_1 = 37, LS_2 = 33, LS_3 = 22, LS_4 = 31;       // Drive Limit Switcher   // Check
...
/////////////// Sabertooth //////////////////////////////
SabertoothSimplified golden_rack(Serial3); // Use SWSerial as the serial port.
...
long min_base_val[40] =           {   0,     0,      0,      0,    0,    0,  -1450,   -840,    0,   0,  -100,    0,  -700,     0,     0,   -1200,   0};
long max_base_val[40] =           { 200,  1700,   1000,      0,  100,    0,      0,     1000,     1450,   0,     0,    0,     0,     0,     0,     0,     100};
long acceleration_val[40] =       {   3,     4,     10,      0,    50,    0,    -11,    10,    11,   0,     -50,    0,    -6,     0,     0,     -6,   50};
long deacceleration_val[40] =     {   0,   -14,    -10,      0,    0,     0,    10,   -10,   -10,   0,      0,    0,     0,     0,     0,    6,      0};
int dis_2_slow[40] =              {   0, 15000,   6000,      0,    0,    0,   4000,  6000,  6000,   0,     0,    0,  3000,     0,     0, 16000,     0};
int time_2_slow[40] =             {   0,  3500,   1000,      0,    0,    0,    1000,  1500,  1000,   0,    0,    0,  1000,     0,     0,  2300,     0};
...
/////////////////////////// Ultrasonic //////////////////
float duration, cm;
int US_manual = 42, US_manual_reverse_trig = A5 , US_manual_reverse_echo = 41, US_ball_detect = 46;
...
int manual_reverse_dis_min, manual_reverse_dis_max, manual_reverse_dis_min_2= 1, manual_reverse_dis_max_2 = 100;
/////////////////////////// Throw ///////////////////////
...
int TZ1_dis_2_move = 2600, TZ2_dis_2_move = 300, dis_margin =200, encoder_control = 0;
...
//    delay(time_2_detach);
    ...
    else if (absolute(xEnc.read() - x_finalpos_tz3) > dis_margin)
    {
      while(absolute(xEnc.read() - x_finalpos_tz3) > dis_margin){
        encoder_control = (x_finalpos_tz3 - xEnc.read())/5;
        junc_move_x(encoder_control);
      }
      stop_bot();
    }
  ...
  {
    if (!junc){
      if (val1 < lsa_star_val && start_read_0 == 0){
        start_read_0 = 1;
        acceleration = -15;
        set_K();
  ...
  if(debug_enable){
    ...
    Serial.print(junc);
    Serial.print(" Lin: ");
    Serial.print(lin_control);
    Serial.print(" Ang: ");
    Serial.println(ang_control);
  }
}

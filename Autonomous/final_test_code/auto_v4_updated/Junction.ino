void junction()
{
  if ((state != 4) && (state != 6) && (state != 9) && (state != 3) && (state != 10)&& (state ! = 12))
  {
...
void check_line(int check_type){
  if (state == 0){
    ...
    if (check_type == 0){
      ...
      while ((analogRead(LSA_2) > lsa_star_val))//||(analogRead(LSA_2) < 300))
      {delay(1);}
    }
  }else if (state == 6){
    if (check_type == 0){
      ...
      while(analogRead(LSA_0) > lsa_star_val || analogRead(LSA_1) > lsa_star_val)
      {
        delay(1);
      }
    }

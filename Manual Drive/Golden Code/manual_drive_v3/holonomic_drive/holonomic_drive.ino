...
int drive_kang_ramp_1 = 330;        // kang ramp              |---------------------|
int drive_kang_ramp_2 = 350;        // kang ramp              | Change in loop also |
int drive_kang_ramp_3 = 830;        // kang ramp              |---------------------|
...
int speed_factor = 15;
int speed_factor_initial = 18; // |-----Change in loop---|
int speed_factor_final = 18;   // |---------also---------|

float x_vel_factor = 1;
float y_vel_factor = 1;

float cor_fact_1 = 0.945;
float cor_fact_2 = 1;
float cor_fact_3 = -0.95;
...
void loop() {
  ...
  if (update_state){
    ...
    automatic_motion();             // Add aut   omatic velocities
    ...
    if (fine_control){                             // Reduce speed and ramp factor for fine manual control
      ...
      speed_factor_initial = 18;
      speed_factor_final = 18;
      manual_ramp = false;
    }
    if (manual_ramp){
      ...
      if (vx == 0 && vy == 0 && ps_w == 0){
        ...
        drive_kang_ramp_3 = 160;
         ...
         drive_kang_ramp_1 = 330;
         drive_kang_ramp_2 = 350;
         drive_kang_ramp_3 = 830;
      }
    }
